...
const PermissionsCrudInternal = () => {
  ...
  const [editingPerm, setEditingPerm] = useState(null); // Guarda el objeto de permiso que se está editando
  const [permissionName, setPermissionName] = useState("");
  const [permissionDesc, setPermissionDesc] = useState(""); // Nuevo campo para la descripción del permiso
  const [permSearch, setPermSearch] = useState(""); // Estado para el campo de búsqueda de permisos
  ...
  const handleOpenPermDialog = (perm = null) => {
    ...
    setPermissionDesc(perm ? (perm.description || "") : ""); // Set description
